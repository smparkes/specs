 ...
 * This is because the implicit defs in the <code>SpecificationBuilder</code> trait
...
trait MapMatchers {
   ...
   */   
  def haveKey[S](k: S) = make[Iterable[(S, Any)]]((map: Iterable[(S, Any)]) => (map.exists{ p => p._1 == k}, map + " has key " + q(k), map + " hasn't key " + q(k))) 
   ...
   */   
  def haveValue[S](v: S) = make[Iterable[(Any, S)]]((map: Iterable[(Any, S)]) => (map.exists(p => p._2 == v), map + " has value " + q(v), map.toString + " hasn't value " + q(v))) 
   ...
   */   
  def havePair[S, T](p: (S, T)) = make[Iterable[(S, T)]]((map: Iterable[(S, T)]) => (map.exists{case e => e == p}, map + " has pair " + q(p), map + " hasn't pair " + q(p)))
